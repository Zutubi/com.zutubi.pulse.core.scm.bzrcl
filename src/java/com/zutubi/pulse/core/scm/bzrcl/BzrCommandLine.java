...
import static com.zutubi.pulse.core.scm.bzrcl.BzrConstants.COMMAND_SVN;
import static com.zutubi.pulse.core.scm.bzrcl.BzrConstants.FLAG_NON_INTERACTIVE;
import static com.zutubi.pulse.core.scm.bzrcl.BzrConstants.FLAG_PASSWORD;
import static com.zutubi.pulse.core.scm.bzrcl.BzrConstants.FLAG_USER;
...
/**
 * A wrapper around the 'bzr' command line utility.  Handles running the
 * external process, and providing global options such as credentials.
 */
public class BzrCommandLine
{
	private BzrConfiguration config;

    public BzrCommandLine(BzrConfiguration config)
    {
        this.config = config;
    }

    /**
     * Runs an external bzr command with the given arguments.  The arguments
     ...
     * @param arguments arguments to pass to the bzr command, e.g.
     *                  {@code {"info", "bzr://myserver/myproject"}}
    ...
    public List<String> run(final ScmFeedbackHandler handler, String... arguments) throws ScmException
    {
    	...
    	ScmProcessRunner runner = new ScmProcessRunner("bzr");
